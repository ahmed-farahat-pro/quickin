CREATE OR REPLACE FUNCTION public.search_listings(p_location text DEFAULT NULL::text, p_geo_lat double precision DEFAULT NULL::double precision, p_geo_lng double precision DEFAULT NULL::double precision, p_geo_radius_km double precision DEFAULT NULL::double precision, p_specific_ids uuid[] DEFAULT NULL::uuid[], p_country text DEFAULT NULL::text, p_include_surrounding boolean DEFAULT true, p_category_slug text DEFAULT NULL::text, p_property_type_slugs text[] DEFAULT NULL::text[], p_guests integer DEFAULT NULL::integer, p_price_min numeric DEFAULT NULL::numeric, p_price_max numeric DEFAULT NULL::numeric, p_check_in date DEFAULT NULL::date, p_check_out date DEFAULT NULL::date, p_attribute_codes text[] DEFAULT NULL::text[], p_best_offer boolean DEFAULT false, p_limit integer DEFAULT 12, p_offset integer DEFAULT 0, p_locale text DEFAULT 'en'::text)
 RETURNS TABLE(id uuid, user_id uuid, title text, description text, price_per_night numeric, location text, city text, state text, country text, max_guests integer, bedrooms integer, beds integer, bathrooms integer, property_type_id uuid, is_guest_favorite boolean, is_published boolean, cleaning_fee numeric, currency text, cancellation_policy text, listing_code character, created_at timestamp with time zone, updated_at timestamp with time zone, avg_rating numeric, review_count bigint, lat double precision, lng double precision, best_offer_price numeric, display_price numeric, total_price numeric, num_nights integer, host_json jsonb, property_type_json jsonb, lifestyles_json jsonb, images_json jsonb, total_count bigint)
 LANGUAGE plpgsql
 STABLE
AS $function$
DECLARE
...
BEGIN
  ...
  WITH filtered AS (
    ...
    LEFT JOIN public.countries ctr ON ctr.id = l.country_id
    LEFT JOIN public.cities cty ON cty.id = l.city_id
    WHERE l.is_published = TRUE
      ...
      AND (
        ...
        OR (
          ...
          OR COALESCE(cty.translations->>p_locale, cty.name) ILIKE '%' || p_location || '%'
          OR cty.name ILIKE '%' || p_location || '%'
          OR COALESCE(ctr.translations->>p_locale, ctr.name) ILIKE '%' || p_location || '%'
          OR ctr.name ILIKE '%' || p_location || '%'
        )
      )
      -- Category filter
  ...
  SELECT
    ...
    l.price_per_night,
    l.location,
    COALESCE(cty.translations->>p_locale, cty.name)::text AS city,
    COALESCE(stt.translations->>p_locale, stt.name)::text AS state,
    COALESCE(ctr.translations->>p_locale, ctr.name)::text AS country,
    l.max_guests,
    l.bedrooms,
    l.beds,
    l.bathrooms,
  ...
  LEFT JOIN public.countries ctr ON ctr.id = l.country_id
  LEFT JOIN public.states stt ON stt.id = l.state_id
  LEFT JOIN public.cities cty ON cty.id = l.city_id
  -- Host
