...
| **Account deletion (5.1.1(v))** | **MISSING.** No in-app delete-account action, no DELETE call (`ProfileView.swift:375–381` has logout only; `ProfileSettingsView` has no delete). Repo grep for delete/deactivate found nothing in app code. | Add in-app "Delete my account" that deletes account + data (or starts the deletion request). Required because the app supports account creation (email + Google). | **Blocker** |
| **Login services (4.8)** | **Removed on purpose.** Sign in with Apple is gone from every platform; the app offers its own email/password account plus Google. 4.8 only bites when a *third-party* service is the exclusive login, and our first-party email login (name + email only, no ad tracking) is the alternative it asks for. | Nothing to enable. If email/password is ever dropped, leaving Google as the only path, 4.8 applies again and an equivalent privacy-focused option must come back. | Watch |
...
6. Verify **iOS permission usage strings** are specific (`Generated/Info.plist`).
